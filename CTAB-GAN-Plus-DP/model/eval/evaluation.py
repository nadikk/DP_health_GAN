...
def supervised_model_training(x_train, y_train, x_test, 
                              y_test, model_name, random_state=42):
...
def get_utility_metrics(real_path,fake_paths,scaler="MinMax",classifiers=["lr","dt","rf","mlp"],test_ratio=.20, random_state=42):

    data_real = pd.read_csv(real_path) #.to_numpy()
    data_real = data_real.replace(r'?', -9999999) #Cervical
    data_real = data_real.apply(pd.to_numeric) #Cervical
    data_real = data_real.to_numpy()
    data_dim = data_real.shape[1]

    data_real_y = data_real[:,-1]
    data_real_X = data_real[:,:data_dim-1]
    X_train_real, X_test_real, y_train_real, y_test_real = model_selection.train_test_split(data_real_X ,data_real_y, test_size=test_ratio, stratify=data_real_y,random_state=random_state) 
    ...
    for classifier in classifiers:
      real_results = supervised_model_training(X_train_real_scaled,y_train_real,X_test_real_scaled,y_test_real,classifier,random_state)
    ...
    for fake_path in fake_paths:
      ...
      data_fake = data_fake.replace(np.nan, -9999999) #Cervical
      data_fake = data_fake.apply(pd.to_numeric) #Cervical

      #for col in data_fake.columns:
        #print(pd.unique(data_fake[col]))

      data_fake = data_fake.to_numpy()
      data_fake_y = data_fake[:,-1]
      data_fake_X = data_fake[:,:data_dim-1]
      X_train_fake, _ , y_train_fake, _ = model_selection.train_test_split(data_fake_X ,data_fake_y, test_size=test_ratio, stratify=data_fake_y,random_state=random_state) 
      ...
      all_fake_results = []
      for classifier in classifiers:
        fake_results = supervised_model_training(X_train_fake_scaled,y_train_fake,X_test_real_scaled,y_test_real,classifier,random_state)
    ...
    return diff_results, np.array(all_real_results), np.array(all_fake_results_avg)
