...
class CTABGANSynthesizer:
    def __init__(self,
                 ...
                 batch_size=500,
                 epochs=150,
                 private=False): #batch_size=500/64, epochs=150
                    ...
                    if self.private:
                        ...
                        for name, param in discriminator.named_parameters():
                            param.grad = (clipped_grads[name] + torch.FloatTensor(
                                clipped_grads[name].size()).normal_(0, self.sigma * self.clip_coeff)) / (
                                                     d_real.size(0) / self.micro_batch_size) #.cuda()
